	...
	vim.ui.input({
		...
		if start_date and start_date ~= "" then
			...
			if year and month and day then
				start_timestamp = os.time({ year = tonumber(year), month = tonumber(month), day =
				tonumber(day) })
			else
		...
		vim.ui.input({
			...
			if end_date and end_date ~= "" then
				...
				if year and month and day then
					end_timestamp = os.time({ year = tonumber(year), month = tonumber(month), day =
					tonumber(day) })
				else
					vim.notify("Invalid date format. Use YYYY-MM-DD", vim.log.levels.ERROR)
					return
...
function M.export_time_report_html()
	local billing = require("tick.billing")

	vim.ui.input({
		prompt = "Project name (leave empty for current project): ",
		...
		vim.ui.input({
			prompt = "Start date (YYYY-MM-DD, leave empty for all time): ",
		}, function(start_date_str)
			local start_date = nil
			if start_date_str and start_date_str ~= "" then
				start_date = os.time({
					year = tonumber(start_date_str:sub(1, 4)),
					month = tonumber(start_date_str:sub(6, 7)),
					day = tonumber(start_date_str:sub(9, 10)),
					hour = 0,
					min = 0,
					sec = 0
				})
			...
			vim.ui.input({
				prompt = "End date (YYYY-MM-DD, leave empty for today): ",
			}, function(end_date_str)
				local end_date = nil
				if end_date_str and end_date_str ~= "" then
					end_date = os.time({
						year = tonumber(end_date_str:sub(1, 4)),
						month = tonumber(end_date_str:sub(6, 7)),
						day = tonumber(end_date_str:sub(9, 10)),
						hour = 23,
						min = 59,
						sec = 59
					})
				...
				local html = billing.generate_time_report_html(project_name, start_date, end_date)
				if not html then
				...
				if f then
					...
					vim.fn.system(open_cmd .. " " .. temp_file)
					vim.notify(
					"Time report opened in browser. You can save it as HTML or print to PDF.",
						vim.log.levels.INFO)
				else
					vim.notify("Failed to create HTML report", vim.log.levels.ERROR)
				end
			end)
		end)
	end)
